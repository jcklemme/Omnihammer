# CODEX: CHAOS

## 1. DETACHMENT FACTIONS
A Chaos army is constructed using one or more **DETACHMENTS**. When adding a **DETACHMENT** to your army, you must select its **DETACHMENT FACTION**. This dictates the core units available, how it interacts with the **FORCE ORGANIZATION CHART**, and the specific special rules it utilizes. 

* **Chaos Space Marines:** The elite Traitor Legions and renegade chapters. Units in this detachment utilize the ***Veterans of the Long War*** and **Marks of Chaos** special rules. 
* **The Lost and the Damned:** Massive hordes of mortal cultists, mutants, and traitor Astra Militarum. 
* **Daemonic Incursion:** Pure manifestations of the warp. Unlike Daemons taken as reserves in other detachments, units in this detachment are not required to use the ***Daemonic Summoning*** rule and may deploy normally or use standard **RESERVES**. 
* **Dark Mechanicum:** The corrupted tech-priests of the Eye of Terror. This detachment prioritizes heavy armor, **WALKERS**, and **DAEMON ENGINES**.
* **Traitoris Lance (Chaos Knights):** A specialized super-heavy detachment representing a fallen Knight house or dreadblades. Its Force Organization Chart consists of: 1 Mandatory **HQ** (Must be a Knight Character), 1-4 Optional **Troops** (War Dogs only), and 0-2 Optional **Lord of War** (Standard Knights).

---

## 2. CHAOS FACTION RULES

***Veterans of the Long War***: Models with this special rule increase their Leadership characteristic by +1 (to a maximum of 10) and possess the ***Hatred (Space Marines)*** special rule. Furthermore, any **UNIT** entirely composed of models with this rule is eligible to purchase **VETERAN SKILLS**. 

**Note on Hatred targets:** For the purposes of this rule, "Space Marines" includes any **UNIT** drawn from Codex: Space Marines, Codex: Blood Angels, Codex: Dark Angels, Codex: Space Wolves, or the Indomitus Crusade Codex.

#### **VETERAN SKILLS**
Units with access to **VETERAN SKILLS** may purchase upgrades from the following list of Universal Special Rules at the points cost indicated in their unit entry. 
* ***Counter-Attack***
* ***Furious Charge***
* ***Infiltrate***
* ***Monster Hunter***
* ***Relentless***
* ***Tank Hunters***

***Daemonic Summoning***: Units with this special rule cannot be deployed on the battlefield at the start of the game and must be placed in **RESERVE**. When a unit with this rule arrives from **RESERVE**, it must enter play using the **DEEP STRIKE** rules with the following modifications:
* The **CENTER MODEL** of the **DEEP STRIKE** must be placed within 6" of a friendly model equipped with a **Personal Icon** that was on the battlefield at the start of the current turn. *(Note: All models that possess the **Daemon** Universal Special Rule inherently count as having a Personal Icon).*
* The unit does not roll for **SCATTER**. This replaces the normal **DEEP STRIKE** scatter procedure described in the Core Rules — no scatter dice or 2D6" are rolled. If the placement would otherwise cause a **DEEP STRIKE MISHAP** (e.g. the center model would land on impassable terrain, off the table, or within 2" of an enemy model), the unit is instead destroyed.. 
* **If there are no friendly models with a Personal Icon on the battlefield when the unit arrives from reserve, the unit is destroyed.**

***Daemonic Possession (Greater Daemons)***: Greater Daemons are kept in **RESERVE** and do not roll to arrive normally. Instead, you must designate a friendly model with the **Champion of Chaos** special rule as the vessel before deployment. At the start of your **MOVEMENT PHASE** from Turn 2 onwards, you must roll a D6 for the vessel. On a 4+, the Greater Daemon bursts forth. If the Greater Daemon has not arrived by Turn 4, it arrives automatically at the start of that turn's Movement Phase. 
When the Greater Daemon arrives:
* Remove the vessel from the battlefield as a casualty. No saves of any kind are allowed. This removal does not count as a casualty for the purposes of Casualty Tests — the vessel's unit does not need to check for having lost 25% or more of its strength as a result of this removal.
* Place the Greater Daemon model so its base is occupying the exact position the vessel was in. The Greater Daemon counts as having arrived from **RESERVE** this turn.

***Daemonic Possession (Vehicles)***: A **VEHICLE** with this special rule ignores the effects of **CREW SHAKEN** and **CREW STUNNED** damage results on a D6 roll of a 2+. However, the vehicle suffers a -1 modifier to its **BALLISTIC SKILL** characteristic (to a minimum of BS 1).

---

## 3. MARKS OF CHAOS

A **UNIT** may only ever have one Mark of Chaos. If a **UNIT** purchases a Mark, all models in the **UNIT** must be upgraded.

#### **Infantry and Walker Effects**
* **Mark of Khorne:** Models gain +1 Attack and ***Furious Charge***.
* **Mark of Nurgle:** Models gain +1 Toughness.
* **Mark of Slaanesh:** Models gain +1 Initiative and ***Feel No Pain (6+)***.
* **Mark of Tzeentch:** Models gain a 6+ **INVULNERABLE SAVE**. If the model already has an **INVULNERABLE SAVE**, it is improved by +1 (max 3+).
* **Mark of Chaos Undivided:** Models may re-roll failed **MORALE**, **PINNING**, and **BREAK TESTS**. 

#### **Non-Walker Vehicle Effects**
* **Mark of Khorne:** Any enemy unit that successfully completes a **CHARGE** against this vehicle immediately suffers D6 Strength 4 AP- hits before melee attacks are resolved.
* **Mark of Nurgle:** The vehicle gains **+1 STRUCTURE POINT**. 
* **Mark of Slaanesh:** Enemy units within 12" of the vehicle cannot perform **STAND & SHOOT!** or **RETURN FIRE!** reactions.
* **Mark of Tzeentch:** The vehicle gains a 6+ **COVER SAVE** against shooting. If already receiving a **COVER SAVE**, improve the save by +1.
* **Mark of Chaos Undivided:** The vehicle automatically passes Leadership tests required to **SPLIT FIRE**.

---

## 4. KHORNE DAEMONS SPECIAL RULES

***Daemon of Khorne:*** A model with this rule is a Daemonic entity of the Blood God. They possess the following Universal Special Rules:
* ***Daemon***
* ***Furious Charge***
* ***Rage***
* ***Fleet***
* ***Adamantium Will***

#### **The Blood God's Due**
In the **ASSAULT PHASE**, if a unit entirely composed of ***Daemon of Khorne*** models wipes out an enemy unit and performs an **OVERRUN!** consolidation, add +1" to the result. **If the die roll is a 1, the unit immediately suffers D3 automatic hits resolved at the unit's majority Strength and AP.**

---

## 5. SPECIFIC LEGION: WORLD EATERS

A **Chaos Space Marines** detachment may be fielded as a **World Eaters** detachment. 

* **The Eightfold Path:** Every unit eligible for a Mark of Chaos must purchase the **Mark of Khorne**. 
* **No Sorcery:** The detachment may not include any **PSYKERS**. 
* **Blood for the Blood God:** Khorne Berserkers are **TROOPS** choices. You must include at least two units of Khorne Berserkers. 
* **Daemonic Allies:** All summoned Daemons must be ***Daemons of Khorne***. 
* **Blood Frenzy:** Units in this detachment never take **PINNING TESTS** and automatically pass **CASUALTY TESTS**. 
* **The Nails:** At the start of the **MOVEMENT PHASE**, if an enemy unit is in line of sight, the unit must make a **NORMAL MOVE** towards the closest visible enemy unit. This compelled move is always a Normal Move, never an Advance Move, and does not prevent the unit from charging in the Assault Phase. In the **ASSAULT PHASE**, if an enemy unit is within charge distance, the unit must declare a **CHARGE** against the closest eligible target.

---

## 6. CHAOS WARGEAR

***Restricted (Mark):*** Some weapons in the Chaos armory are favored by a specific dark god. If a weapon has the ***Restricted*** special rule followed by a specific Mark (e.g., *Restricted (Khorne)*), it may only be equipped by a model that possesses that exact Mark of Chaos, or by a model in a detachment entirely dedicated to that god (such as World Eaters). 

#### **Ranged Weapons**
| Weapon | Range | Str | AP | Type |
| :--- | :---: | :---: | :---: | :--- |
| **Bolt Pistol** | 12" | 4 | 5 | Pistol |
| **Hand Flamer** | Template | 3 | 6 | Pistol |
| **Bolter** | 24" | 4 | 5 | Rapid Fire |
| **Combi-Bolter** | 24" | 4 | 5 | Rapid Fire, ***Twin-Linked*** |
| **Plasma Pistol** | 12" | 7 | 2 | Pistol, ***Gets Hot!*** |
| **Lash of Khorne** | 12" | 6 | 3 | Assault 2, ***Fleshbane***, ***Restricted (Khorne)*** |
| **Bloodflail** | 8" | 8 | 2 | Assault 1, ***Concussive***, ***Restricted (Khorne)*** |

#### **Melee Weapons**
| Weapon | Str | AP | Special Rules |
| :--- | :---: | :---: | :--- |
| **Chainsword** | User | - | ***Melee***, ***Shred*** |
| **Chainaxe** | User | 4 | ***Melee***, ***Shred*** |
| **Power Sword** | User | 2 | ***Melee***, ***Power Weapon*** |
| **Power Axe** | +1 | 2 | ***Melee***, ***Power Weapon***, ***Unwieldy*** |
| **Power Maul** | +2 | 2 | ***Melee***, ***Power Weapon***, ***Concussive*** |
| **Power Spear** | +1/User* | 2 | ***Melee***, ***Power Weapon*** |
| **Power Fist** | x2 | 2 | ***Melee***, ***Power Weapon***, ***Unwieldy***, ***Specialist Weapon*** |
| **Lightning Claw** | User | 2 | ***Melee***, ***Power Weapon***, ***Shred***, ***Specialist Weapon*** |
| **Khornate Eviscerator** | x2 | 2 | ***Melee***, ***Shred***, ***Two-Handed***, ***Unwieldy***, ***Armorbane***, ***Restricted (Khorne)*** |
| **Meteor Hammer**** | +2 | 3 | ***Melee***, ***Two-Handed***, ***Concussive***, ***Restricted (Khorne)*** |
| **Twin Falax Blades***** | User | 3 | ***Melee***, ***Shred***, ***Specialist Weapon***, ***Restricted (Khorne)*** |
| **Great Axe of Khorne** | x2 | 2 | ***Melee***, ***Two-Handed***, ***Armorbane***, ***Terrible Weapon***, ***Unwieldy***, ***Restricted (Khorne)*** |

* Power Spear: The +1 Strength value applies only on the turn the model charges; in subsequent rounds of combat, use the User Strength value.
** Meteor Hammer: The wielder gains +1 Initiative on the turn they charge.
*** Twin Falax Blades: These grant +2 Attacks for wielding two melee weapons (instead of the standard +1 Attack bonus).

#### **Daemon Weapons**
* **Mastery Test:** At the start of each **ASSAULT PHASE** in combat, roll a **D6**. Add the result to the model's **Attack** characteristic for the phase. 
* **The Rebellion:** If the Mastery Test is a **1**, the wielder immediately suffers an **Unsaved Wound** (no saves allowed) and makes no attacks for the remainder of that phase.
* **Two-Handed & Warp-Sensed:** All Daemon Weapons are ***Two-Handed*** and their attacks count as **Magic/Warp**.

| Weapon | Str | AP | Special Rules |
| :--- | :---: | :---: | :--- |
| **Axe of Khorne** | User | 2 | ***Melee***, ***Power Weapon***, ***Daemon Weapon***, ***Beheading Strike***, ***Restricted (Khorne)*** |
| **Berzerker Glaive** | +1 | 2 | ***Melee***, ***Power Weapon***, ***Daemon Weapon***, ***Driven by the Nails***, ***Restricted (Khorne)*** |

* ***Beheading Strike:*** To Hit rolls of a natural **6** generate one additional attack. These bonus attacks can continue to generate further attacks if they roll a **6**.
* ***Driven by the Nails:*** The wielder may never join a unit and loses the **Independent Character** rule. They gain ***Feel No Pain (4+)*** and ***Fleet***. The model must always Advance towards the nearest enemy unit in the Movement Phase and must declare a charge if an enemy is in range.

---

## 7. DAEMONIC GIFTS
Characters with the **CHAMPION OF CHAOS** rule may spend their points allowance on the following gifts. These may only be taken once per model.

* **Daemonic Armour (X pts):** The model's Armor Save is improved to 2+.
* **Daemonic Aura (X pts):** The model gains a 5+ **INVULNERABLE SAVE**. *(If the model already has an Invulnerable Save, it is improved by 1, to a maximum of 3+).*
* **Daemon Chains (X pts):** If this model is designated as the vessel for the ***Daemonic Possession (Greater Daemons)*** special rule, you may re-roll the D6 result to determine if the Greater Daemon arrives this turn.
* **Daemonic Essence (X pts):** The model gains +1 Wound.
* **Daemonic Fire (X pts):** The model gains a breath weapon that can be fired in the Shooting Phase: *(Range: Template, Str 4, AP 5, Assault 1)*. 
* **Daemonic Flight (X pts):**  The model's unit type changes to **JUMP INFANTRY**, replacing all previous movement rules for the model with those of the Jump Infantry type modifier as described in the Core Rules Unit Types. _(If the model is a Monstrous Creature, its unit type changes to **FLYING MONSTER**, replacing all previous movement rules with those of the Flying Monsters type.)_
* **Daemonic Mutation (X pts):** The model gains +1 Attack.
* **Daemonic Resilience (X pts):** The model gains +1 Toughness.
* **Daemonic Rune (X pts):** The model gains the ***Eternal Warrior*** special rule (immune to Massive Wounds).
* **Daemonic Stature (X pts):** The model's unit type changes to **MONSTROUS CREATURE**. The model loses the **INDEPENDENT CHARACTER** special rule — it may no longer join other units, and other units may no longer join it. _(Only available to Chaos Lords, Sorcerer Lords, and Daemon Princes)._
* **Daemon Spawn (X pts):** If this model kills an enemy Character in a melee engagement, you may immediately place a **Chaos Spawn** model in base contact with the slain enemy before removing the casualty. This Spawn is under your control for the rest of the game.
* **Daemon Steed (X pts):** The model is mounted on a daemonic beast. The model's unit type changes to **CAVALRY**, replacing all previous movement rules for the model with those of the Cavalry type as described in the Core Rules Unit Types. The model gains +1 Toughness and the **_Fear_** special rule. 
* **Daemonic Speed (X pts):** The model adds +6" to its **NORMAL MOVE** distance only. This bonus does not affect Advance Move distances, charge distances, fallback distances, or any other movement type.
* **Daemonic Strength (X pts):** The model gains +1 Strength.
* **Daemonic Talons (X pts):** The model is equipped with a melee weapon with the following profile: *(Str User, AP 2, **Melee**, **Double Weapon**, **Rending**, **Armorbane**).*
* **Daemon Venom (X pts):** The model may not have any other melee weapons. Its close combat attacks gain the ***Poison (2+)*** special rule. 
* **Daemonic Visage (X pts):** The model gains the ***Fear*** special rule.

---

## 8. MISCELLANEOUS GEAR
* **Personal Icon:** A model equipped with a Personal Icon serves as a beacon. Friendly units with ***Daemonic Summoning*** may Deep Strike within 6" of this model without scattering. 
* **Collar of Khorne:** Grants the equipped model and their unit the ***Adamantium Will*** special rule. May only be taken by models with the **Mark of Khorne**.
* **Icon of Wrath:** A unit equipped with this icon may re-roll failed **Charge Distance** rolls. May only be taken by a unit with the **Mark of Khorne**.
* **Instrument of Chaos:** If a melee engagement results in a tie (both sides inflicted an equal number of unsaved wounds), a unit equipped with an Instrument of Chaos counts as having won the combat by 1. This only applies if at least one unsaved wound was inflicted during the phase. If the opposing unit also possesses a rule that allows it to win tied combats, the rules cancel out and the combat is resolved as a tie.
* **Daemonic Icon:** When a unit equipped with a Daemonic Icon fails a **Daemonic Instability Test**, reduce the number of unsaved wounds suffered by 1. Furthermore, if the unit takes a Daemonic Instability Test and rolls a natural 2, return D3 slain models to the unit.

---

## 9. UNIT PROFILES

### KHORNE BERZERKERS

| Model | WS | BS | S | T | W | I | A | Ld | Save | Inv | Type |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| Khorne Berzerker | 4 | 4 | 4 | 4 | 1 | 4 | 1 | 8 | 3+ | - | Infantry |
| Berzerker Champion | 5 | 4 | 4 | 4 | 1 | 4 | 2 | 9 | 3+ | - | Infantry (Character) |

**Unit Composition:** * 1 Berzerker Champion
* 4-19 Khorne Berzerkers

**Force Organization Role:** * Elites (Chaos Space Marines Detachment)
* Troops (World Eaters Detachment)

**Wargear:**
* Power Armor
* Bolt Pistol
* Chainsword or Chainaxe
* Frag Grenades
* Krak Grenades

**Special Rules:**
* ***Veterans of the Long War***
* **Mark of Khorne** *(Grants +1 Attack and Furious Charge)*
* ***Fearless***
* ***Rage***

**Options:**
* Any model may replace its Bolt Pistol with a Chainsword or Chainaxe (Free).
* Any model may replace its Bolt Pistol and melee weapon with a Bolter (Free).
* For every 5 models in the unit, one Khorne Berzerker may replace their Bolt Pistol with a Plasma Pistol or Hand Flamer for X pts.
* For every 5 models in the unit, one Khorne Berzerker may replace their melee weapon with a Khornate Eviscerator for X pts.
* The unit may take a Collar of Khorne for X pts. 
* One Khorne Berzerker may take a Personal Icon for X pts or an Icon of Wrath for X pts.
* The Berzerker Champion may replace his Bolt Pistol and/or melee weapon with items from the Chaos Melee Weapons or Ranged Weapons lists.
* The Berzerker Champion may take Melta Bombs for X pts.
* The Berzerker Champion may take up to X points of Daemonic Gifts.

### BLOODLETTERS

| Model | WS | BS | S | T | W | I | A | Ld | Save | Inv | Type |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| Bloodletter | 5 | 0 | 4 | 4 | 1 | 4 | 1 | 10 | - | 5++ | Infantry |
| Bloodreaper | 5 | 0 | 4 | 4 | 1 | 4 | 2 | 10 | - | 5++ | Infantry (Character) |

**Unit Composition:** * 8-20 Bloodletters

**Force Organization Role:** * Troops (Daemonic Incursion Detachment)
* Elites (All other Chaos Detachments)

**Wargear:**
* Hellblade (S User, AP 3, ***Melee***, ***Shred***)

**Special Rules:**
* ***Daemon of Khorne*** *(Grants Daemon, Furious Charge, Rage, Fleet, Adamantium Will)*
* ***Daemonic Summoning*** *(Requires Deep Strike via Personal Icon in non-Daemon detachments. Note: All models with the Daemon USR count as having a Personal Icon.)*

**Options:**
* One Bloodletter may be upgraded to a Bloodreaper for X pts.
* One Bloodletter may carry an Instrument of Chaos for X pts.
* One Bloodletter may carry a Daemonic Icon for X pts.

### FLESH HOUNDS

| Model | WS | BS | S | T | W | I | A | Ld | Save | Inv | Type |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| Flesh Hound | 5 | 0 | 4 | 4 | 2 | 4 | 2 | 10 | - | 5++ | Beasts |
| Gore Hound | 5 | 0 | 4 | 4 | 3 | 4 | 3 | 10 | - | 5++ | Beasts (Character) |

**Unit Composition:** * 5-20 Flesh Hounds

**Force Organization Role:** * Fast Attack (All Chaos Detachments)

**Wargear:**
* Massive Claws and Teeth (S User, AP 5, ***Melee***, ***Shred***)
* Innate Collar of Khorne

**Special Rules:**
* ***Daemon of Khorne*** *(Grants Daemon, Furious Charge, Rage, Fleet, Adamantium Will)*
* ***Daemonic Summoning***
* ***Acute Senses***
* ***Scouts***
* **Hounds of the Blood God:** Flesh Hounds possess an Innate Collar of Khorne. A unit entirely composed of models with this rule passes **Deny the Witch** tests on a roll of 3+ instead of the standard 6+.

**Options:**
* One Flesh Hound may be upgraded to a Gore Hound for X pts.
    * **The Gore Hound's Roar:** The Gore Hound possesses a breath weapon with the following profile: *(Range: Template, Str 4, AP 5, Assault 1, **Soul Blaze**)*. Note: Because Template weapons automatically hit all models under the template, the Gore Hound may fire this weapon despite having a Ballistic Skill of 0.
    * **Scent of Burning Flesh:** If an enemy unit is currently suffering the effects of the ***Soul Blaze*** special rule, this unit gains the ***Preferred Enemy*** special rule against them.

### BLOODTHIRSTER

| Model | WS | BS | S | T | W | I | A | Ld | Save | Inv | Type |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| Bloodthirster | 10 | 5 | 8 | 6 | 5 | 9 | 5 | 10 | 3+ | 4++ | Flying Monsters (Character) |

**Unit Composition:** * 1 Bloodthirster

**Force Organization Role:** * HQ (Daemonic Incursion Detachment)
* HQ (All other Chaos Detachments, subject to Daemonic Possession limits)

**Wargear:**
* Brass Armour of Khorne (Provides 3+ Armor Save, 4+ Invulnerable Save)
* Axe of Khorne
* Lash of Khorne

**Special Rules:**
* ***Daemon of Khorne*** *(Grants Daemon, Furious Charge, Rage, Fleet, Adamantium Will)*
* ***Daemonic Possession (Greater Daemons)*** *(In non-Daemon Detachments)*
* ***Resistant to Small Arms***
* **The 8th Host (Unfettered Fury):** The Bloodthirster is armed with an Axe of Khorne and a Lash of Khorne (included in profile).

**Options:**
* **Host Upgrades (Choose One):**
    * **The 6th Host (Insensate Rage) (X pts):** Replace the Axe of Khorne and Lash of Khorne with a **Great Axe of Khorne**. Additionally, friendly units with the ***Daemon of Khorne*** special rule within 12" gain +2" to their charge distances.
    * **The 3rd Host (Wrath of Khorne) (X pts):** Replace the Lash of Khorne with a **Bloodflail**. The Bloodthirster gains the ***Preferred Enemy (Characters)*** special rule.
